...
try{
 for(const script of ['migrate','seed','migrate','seed']){
  ...
  process.stdout.write(result.stdout);assert.equal(result.status,0,result.stderr);
 }
 ...
 const expected=['places','partners','experiences','events','categories','sources'].reduce((n,k)=>n+(fixtures as unknown as Record<string,unknown[]>)[k].length,0);
 ...
 await db.close();
 const seedAgain=spawnSync(process.execPath,['--import','tsx','scripts/seed.ts'],{env,encoding:'utf8'});assert.equal(seedAgain.status,0,seedAgain.stderr);
 ...
 console.log(`PASS: migration repeated; ${expected} content records and 1 trip; seed repeated three times; edited version preserved after reopen.`);
