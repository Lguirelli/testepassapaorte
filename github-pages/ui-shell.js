(() => {
  ...
  const brand=()=>`<a class="brand" href="#/" aria-label="Passaporte Serra Negra, página inicial"><span class="brand-wordmark" data-brand-wordmark="true" aria-hidden="true"></span></a>`;
  const nav=C.navigation.main.map(item=>`<a class="nav-icon-link ${item.cta?'button primary nav-cta':''}" href="${item.href}" data-nav-key="${esc(item.label)}" aria-label="${esc(item.label)}"><span class="nav-dock-motion" data-dock-motion>${I.html(item.iconKey,'',{size:26})}<span class="nav-hover-label" aria-hidden="true">${esc(item.label)}</span></span></a>`).join('');
