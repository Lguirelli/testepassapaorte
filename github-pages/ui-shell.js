(() => {
  ...
  const nav=C.navigation.main.map(item=>`<a class="${item.cta?'button primary nav-cta':''}" href="${item.href}" data-nav-key="${esc(item.label)}"><span class="nav-dock-motion" data-dock-motion>${I.html(item.iconKey,'',{size:18})}<span>${esc(item.label)}</span></span></a>`).join('');
