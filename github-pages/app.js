(() => {
  ...
  const ROUTES = [
    {match:p=>p[0]==='roteiros', meta:'roteiros', run:()=>window.PSN_READY_ROUTES?.render(), customMeta:true},
  ...
  function render(){
    ...
    if(window.PSN_REFERENCE_PAGES?.render(hash,app)){if(page==='home'||page==='parceiros')setPageMeta(page==='home'?'home':'parceiros');syncPremiumHeader();app.focus({preventScroll:true});return;}
