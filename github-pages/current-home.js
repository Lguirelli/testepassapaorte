(() => {
  ...
  function renderHome(){
    ...
    app.innerHTML=`<div class="current-home" data-current-home="true">
      ...
      <section class="ch-section ch-route-types"><div class="ch-shell"><div class="ch-center"><p class="ch-eyebrow">ROTEIROS PARA COMEÇAR</p><h2>Escolha uma boa base antes de pensar em cada detalhe.</h2><p>Os roteiros partem de situações reais de viagem. Você escolhe um, cria sua própria versão e muda o que quiser depois.</p></div><div class="ch-tabs" role="tablist" aria-label="Roteiros prontos">${readyRoutes.map((r,i)=>`<button class="ch-tab" role="tab" aria-selected="${i===0}" tabindex="${i===0?0:-1}" data-route-tab="${i}">${esc(r.eyebrow)}</button>`).join('')}</div><div data-route-slide></div><div class="ch-actions" style="justify-content:center"><a class="ch-button" href="#/roteiros">Ver todos os roteiros</a><a class="ch-button" href="#/roteiro">Criar do zero</a></div></div></section>
