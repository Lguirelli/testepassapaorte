(() => {
  ...
  const isHome=()=>!window.PSN_REFERENCE_PAGES&&['','/'].includes(path());
  ...
  function renderHome(){
    ...
    app.innerHTML=`<div class="current-home" data-current-home="true">
      ...
      <section class="ch-section"><div class="ch-shell ch-passport"><div class="ch-passport-paper" data-passport-cover="true" aria-hidden="true"><img src="./assets/brand/passport/passaporte-capa-couro.png" alt=""></div><div><p class="ch-eyebrow">DE ROTEIRO A MEMÓRIA</p><h2>O plano termina. A experiência fica.</h2><p>O roteiro registra intenção. O Passaporte guarda os lugares que realmente fizeram parte da viagem e transforma o percurso vivido em memória.</p><div class="ch-actions"><a class="ch-button ch-button-primary" href="#/meu-passaporte">Ver meu Passaporte</a><a class="ch-button" href="#/roteiros">Escolher um roteiro</a></div></div></div></section>
  ...
  function bindHome(partnerList){
    ...
    app.querySelector('[data-gallery-prev]')?.addEventListener('click',()=>move(-1));app.querySelector('[data-gallery-next]')?.addEventListener('click',()=>move(1));
    let pointerId=null,startX=0,lastX=0,lastAt=0,velocity=0,dragged=false,suppressClickUntil=0,navigateBypass=false;
    items.forEach((el,i)=>{
      el.addEventListener('dragstart',e=>e.preventDefault());
      el.addEventListener('click',e=>{
        if(navigateBypass||e.detail===0)return;
        e.preventDefault();e.stopPropagation();
        if(dragged||performance.now()<suppressClickUntil){dragged=false;return;}
        if(i!==active){active=i;updateGallery();}
      });
      el.addEventListener('dblclick',e=>{
        if(navigateBypass)return;
        e.preventDefault();e.stopPropagation();
        if(dragged||performance.now()<suppressClickUntil){dragged=false;return;}
        const link=el.querySelector('a[href]');
        if(link){navigateBypass=true;link.click();queueMicrotask(()=>{navigateBypass=false;});}
      });
    });
    gallery?.addEventListener('wheel',e=>{if(Math.abs(e.deltaY)<8)return;e.preventDefault();move(e.deltaY>0?1:-1);},{passive:false});gallery?.addEventListener('keydown',e=>{if(e.key==='ArrowRight'){e.preventDefault();move(1);}if(e.key==='ArrowLeft'){e.preventDefault();move(-1);}});
    gallery?.addEventListener('pointerdown',e=>{if(e.button!==0)return;pointerId=e.pointerId;startX=lastX=e.clientX;lastAt=performance.now();velocity=0;dragged=false;gallery.dataset.dragging='true';gallery.style.setProperty('--drag-shift','0px');gallery.setPointerCapture?.(e.pointerId);});
    gallery?.addEventListener('pointermove',e=>{if(pointerId!==e.pointerId)return;const now=performance.now();const dt=Math.max(1,now-lastAt);velocity=(e.clientX-lastX)/dt;lastX=e.clientX;lastAt=now;const raw=e.clientX-startX;if(Math.abs(raw)>6){dragged=true;e.preventDefault();}const delta=Math.max(-150,Math.min(150,raw*.56));gallery.style.setProperty('--drag-shift',`${delta}px`);});
    const release=e=>{if(pointerId!==e.pointerId)return;const delta=e.clientX-startX;const projected=delta+velocity*180;gallery.style.setProperty('--drag-shift','0px');delete gallery.dataset.dragging;if(dragged)suppressClickUntil=performance.now()+120;if(Math.abs(projected)>48)move(projected<0?1:-1);pointerId=null;};
    gallery?.addEventListener('pointerup',release);gallery?.addEventListener('pointercancel',e=>{if(pointerId!==e.pointerId)return;gallery.style.setProperty('--drag-shift','0px');delete gallery.dataset.dragging;pointerId=null;dragged=false;});updateGallery();
  ...
  window.PSN_READY_ROUTES={render:renderReadyRoutes};
