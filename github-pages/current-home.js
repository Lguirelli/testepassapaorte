(() => {
  ...
  function bindHome(partnerList){
    ...
    let pointerId=null,startX=0,lastX=0,lastAt=0,velocity=0,dragged=false,suppressClickUntil=0,lastClickId='',lastClickAt=0,navigateBypass=false;
    items.forEach((el,i)=>el.addEventListener('click',e=>{
      ...
      const id=el.dataset.sharedPlace||String(i),now=performance.now();const isDouble=id===lastClickId&&now-lastClickAt<=420;
      if(isDouble){lastClickId='';lastClickAt=0;const link=el.querySelector('a[href]');if(link){navigateBypass=true;link.click();queueMicrotask(()=>{navigateBypass=false;});}return;}
      lastClickId=id;lastClickAt=now;if(i!==active){active=i;updateGallery();}
      setTimeout(()=>{if(lastClickId===id&&performance.now()-lastClickAt>=400){lastClickId='';lastClickAt=0;}},440);
    }));
    ...
    const release=e=>{if(pointerId!==e.pointerId)return;const delta=e.clientX-startX;const projected=delta+velocity*190;gallery.style.setProperty('--drag-shift','0px');delete gallery.dataset.dragging;if(dragged)suppressClickUntil=performance.now()+120;if(Math.abs(projected)>48)move(projected<0?1:-1);pointerId=null;};
