...
function PartnerGallery({partners}:{partners:ContentData[]}){
  ...
  const writeDrag=(px:number)=>{if(pointerFrame.current!==null)cancelAnimationFrame(pointerFrame.current);pointerFrame.current=requestAnimationFrame(()=>{stageRef.current?.style.setProperty('--drag-shift',`${Math.max(-150,Math.min(150,px))}px`);pointerFrame.current=null;});};
